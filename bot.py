...
# - Telegram webhook mode is used for Render Web Service deployment.
# ============================================================
...
# Required: the public HTTPS URL of this Render service.
WEBHOOK_URL = os.environ.get("WEBHOOK_URL", "").strip().rstrip("/")
if not WEBHOOK_URL.startswith("https://"):
    raise RuntimeError("WEBHOOK_URL must be set to the Render HTTPS service URL")
...
def main():
    ...
    log.info(
        "Starting ABOD Ludo Bot in webhook mode | tax=%s%% | joining bonus=%s | port=%s",
        GROUP_TAX_PERCENT,
        JOIN_BONUS,
        PORT,
    )
