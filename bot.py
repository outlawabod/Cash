...
def init_db():
    ...
    if old_exists:
        ...
        if {"chat_id", "username", "display_name", "points"}.issubset(old_columns):
            ...
            # Store legacy balances in a small compatibility table.
            conn.execute("""
...
async def manual_adjust(update: Update, context: ContextTypes.DEFAULT_TYPE, sign: int):
    ...
    if sign < 0 and points < amount:
        await reply(
            update,
            f"❌ {name} has only {points} pts. Cannot remove {amount}."
        )
        return
    ...
    await reply(
        update,
        f"{icon} <b>{name}</b> → {sign*amount:+d} pts\n"
...
async def new_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if b1 < stake:
        await reply(
            ...
            f"{p1[2]} has {b1} pts, but table needs {stake} pts."
        )
        return
    ...
    if b2 < stake:
        await reply(
            update,
            f"❌ Balance check failed!\n"
            f"{p2[2]} has {b2} pts, but table needs {stake} pts."
        )
        return
...
async def finish_table_logic(chat_id, table_id, winner_slot, update, context):
    ...
    text = (
        ...
        f"🏆 Winner: <b>{winner[2]}</b>\n"
        f"💰 Table: <b>{stake} + {stake} = {pot}</b> pts\n"
        f"🧾 Group tax: <b>{tax}</b> pts ({GROUP_TAX_PERCENT:g}%)\n"
        f"💵 Winner payout: <b>{payout}</b> pts\n\n"
        f"🥇 {winner[2]} balance: <b>{winner_after}</b> pts\n"
        f"❌ {loser[2]} balance: <b>{loser_after}</b> pts\n"
        f"📊 {winner[2]}: W +1\n"
        f"📊 {loser[2]}: L +1"
    )
    if owner_name:
        text += f"\n👑 Owner service tax: <b>{owner_name}</b> +{tax} pts"
...
async def cancel_table_logic(chat_id, table_id, update, context):
    ...
    text = (
        ...
        f"↩️ {p1[2]}: +{stake} refund\n"
        f"↩️ {p2[2]}: +{stake} refund\n"
...
def main():
    ...
    # Polling is intentionally used because it avoids the old broken
    # WEBHOOK_URL/404 setup. Render's HTTP health server above keeps the
    # Web Service bound to PORT.
    app.run_polling(
