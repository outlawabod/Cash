...
async def callback(update, context):
    """Handle all inline buttons reliably."""
    ...
    # Always acknowledge the Telegram callback immediately so the button
    # never appears to keep loading.
    try:
        await q.answer()
    except Exception:
        pass
    ...
    try:
        ...
        if data.startswith("winner:"):
            if not await telegram_admin(update, context, user_id):
                await q.answer("⛔ Admin only.", show_alert=True)
                return

            parts = data.split(":")
            if len(parts) != 3:
                await q.answer("❌ Invalid winner button.", show_alert=True)
                return
            ...
            if not row or row[0] != chat_id:
                await q.answer("❌ Table not found.", show_alert=True)
                return

            table_no, status = row[1], row[2]
            if status != "pending":
                await q.answer("ℹ️ This table is already closed.", show_alert=True)
                return

            log.info(
                "Winner button: chat=%s table=%s slot=%s admin=%s",
                chat_id, table_no, slot, user_id
            )

            ok, msg = await resolve_winner(
                update, context, table_no, slot, q.message
            )
            if ok:
                await q.answer("🏆 Winner recorded!")
            else:
                await q.answer(msg or "❌ Could not record winner.", show_alert=True)
            return
        ...
        if data.startswith("cancel:"):
            ...
            try:
                table_id = int(data.split(":", 1)[1])
            except (ValueError, IndexError):
                await q.answer("❌ Invalid table button.", show_alert=True)
                return
            ...
            if cur.rowcount:
                try:
                    await q.message.edit_text(
                        f"❌ Table No. {row[1]} cancelled by admin."
                    )
                except Exception as exc:
                    log.warning("Cancel edit failed: %s", exc)
                await q.answer("❌ Table cancelled.")
            else:
                await q.answer("ℹ️ Table already closed.", show_alert=True)
            return

        # ---------------- My points button ----------------
        if data == "my_points_button":
            row = get_or_create_user(chat_id, q.from_user)
            await q.answer(
                f"💰 {row[4]} points\n"
                f"🎮 {row[5]} matches | 🏆 {row[6]} wins | 📉 {row[7]} losses",
                show_alert=True,
            )
            return

        await q.answer("ℹ️ Unknown button.", show_alert=True)

    except Exception:
        log.exception("INLINE BUTTON ERROR | chat=%s user=%s data=%s",
                      chat_id, user_id, data)
        try:
            await q.answer(
                "❌ Button error. Check Render logs.",
                show_alert=True
            )
        except Exception:
            pass
